...
__global__ void median_precompiled_Overlap2DKernel_median_kernel_conv_cuda_2D_kernel(
	unsigned char* input, unsigned long elemPerPx, const int region,  unsigned char* output,
...
)
{
   extern __shared__ unsigned char sdata[]; // will also contain extra (overlap data)
	...
	if (x < out_cols && y < out_rows)
		output[y*out_pitch+x] = skepu_userfunction_skepu_skel_0calculateMedian_median_kernel::CU({(int)overlap_x, (int)overlap_y,
			sharedCols, &sdata[(threadIdx.y + overlap_y) * sharedCols + (threadIdx.x + overlap_x)]} , elemPerPx, region);
}
