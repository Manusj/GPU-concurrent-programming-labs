...
class CLWrapperClass_dotproduct_precompiled_MapKernel_addOneFunc_arity_1
{
public:
...
__kernel void dotproduct_precompiled_MapKernel_addOneFunc_arity_1( __global float* skepu_output, __global float *a,   size_t skepu_n, size_t skepu_base)
{
	...
	while (skepu_i < skepu_n)
	{
		...
		for (skepu::backend::Device_CL *device : skepu::backend::Environment<int>::getInstance()->m_devices_CL)
		{
			...
			cl_kernel kernel = clCreateKernel(program, "dotproduct_precompiled_MapKernel_addOneFunc_arity_1", &err);
			CL_CHECK_ERROR(err, "Error creating map kernel 'dotproduct_precompiled_MapKernel_addOneFunc_arity_1'");
