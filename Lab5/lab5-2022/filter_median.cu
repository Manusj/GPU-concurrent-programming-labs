...
// Filter kernel (simple box filter)
	...
	//0-255
	//array[256]
	//array[shared_mem[(shared_index_x+((shared_index_y)*(blocksize+2*kernelsizex)))*3+0]]++;
	//find index that has max value - median 
	unsigned int pixelcount_x[256];
	unsigned int pixelcount_y[256];
	unsigned int pixelcount_z[256];
	for(int i = 0; i<256; i++)
	{
		pixelcount_x[i] = 0;
		pixelcount_y[i] = 0;
	...
	for(dy=-kernelsizey;dy<=kernelsizey;dy++)
	{
		for(dx=-kernelsizex;dx<=kernelsizex;dx++)	
		{
			...
			pixelcount_x[shared_mem[(shared_index_x+((shared_index_y)*(blocksize+2*kernelsizex)))*3+0]]++;		//now these change
			pixelcount_y[shared_mem[(shared_index_x+((shared_index_y)*(blocksize+2*kernelsizex)))*3+1]]++;		//TODO
			pixelcount_z[shared_mem[(shared_index_x+((shared_index_y)*(blocksize+2*kernelsizex)))*3+2]]++;
	...
	int mid = ((((kernelsizey*2)+1)*((kernelsizey*2)+1))/2)+1;
	if(threadIdx.x==0&&blockIdx.x==0&&threadIdx.y==0&&blockIdx.y==0)
	{
		printf("mid - %d\n", mid);
	}
	int countx=mid, county=mid,countz=mid;

	int medianx=0, mediany=0,medianz=0;
	bool flag_x = true, flag_y = true, flag_z = true;
	for(int i = 0; i<256; i++)
	{
		countx-=pixelcount_x[i];
		if(countx<=0 && flag_x)
		{
			medianx = i;
			flag_x = false;
		}
		county-=pixelcount_y[i];
		if(county<=0 && flag_y)
		{
			mediany = i;
			flag_y = false;
		}
		countz-=pixelcount_z[i];
		if(countz<=0 && flag_z)
		{
			medianz = i;
			flag_z = false;
		}
		if(threadIdx.x==0&&blockIdx.x==0&&threadIdx.y==0&&blockIdx.y==0&&pixelcount_x[i]!=0)
		{
			//printf("pixelcount_x - %d, i - %d", pixelcount_x[i],i);
		}
	}
	if(threadIdx.x==0&&blockIdx.x==0&&threadIdx.y==0&&blockIdx.y==0)
	{
		printf("\nmaxx= %d, pixelcount_x[maxx] - %d\n",medianx, pixelcount_x[medianx]);
		printf("maxy= %d, pixelcount_y[maxx] - %d\n",mediany, pixelcount_y[mediany]);
		printf("maxz= %d, pixelcount_z[maxx] - %d\n",medianz, pixelcount_z[medianz]);
	}
	out[(y*imagesizex+x)*3+0] = medianx;
	out[(y*imagesizex+x)*3+1] = mediany;
	out[(y*imagesizex+x)*3+2] = medianz;
	if(threadIdx.x==0&&blockIdx.x==0&&threadIdx.y==0&&blockIdx.y==0)
