...
__global__ void filter(unsigned char *image, unsigned char *out, const unsigned int imagesizex, const unsigned int imagesizey, const int kernelsizex, const int kernelsizey, int *ptrGpu_arrayFilter)
{ 
...
//initilize shared memory	TODO
	...
	//done
  	...
  	int divby = 16;//(2*kernelsizey+1);// Works for box filters only!
...
// Filter kernel (simple box filter)
	...
	for(dy=-kernelsizey;dy<=kernelsizey;dy++)
	{
		...
		int shared_index_x= min(max(my_shared_mem_index_x+dx, 0), blocksize+2*kernelsizex);
		int shared_index_y= min(max(my_shared_mem_index_y+dy, 0), blocksize+2*kernelsizey);
		sumx += (shared_mem[(shared_index_x+((shared_index_y)*(blocksize+2*kernelsizex)))*3+0]*ptrGpu_arrayFilter[dy+2]);		//now these change
		sumy += (shared_mem[(shared_index_x+((shared_index_y)*(blocksize+2*kernelsizex)))*3+1]*ptrGpu_arrayFilter[dy+2]);		//TODO
		sumz += (shared_mem[(shared_index_x+((shared_index_y)*(blocksize+2*kernelsizex)))*3+2]*ptrGpu_arrayFilter[dy+2]);
	}

	shared_mem[(my_shared_mem_index_x+((my_shared_mem_index_y)*(blocksize+2*kernelsizex)))*3+0] = sumx/divby;
	shared_mem[(my_shared_mem_index_x+((my_shared_mem_index_y)*(blocksize+2*kernelsizex)))*3+1] = sumy/divby;
	shared_mem[(my_shared_mem_index_x+((my_shared_mem_index_y)*(blocksize+2*kernelsizex)))*3+2] = sumz/divby;
	...
	for(dx=-kernelsizex;dx<=kernelsizex;dx++)	
		{
			...
			int shared_index_x= min(max(my_shared_mem_index_x+dx, 0), blocksize+2*kernelsizex);
			int shared_index_y= min(max(my_shared_mem_index_y+dy, 0), blocksize+2*kernelsizey);
			sumx += (shared_mem[(shared_index_x+((shared_index_y)*(blocksize+2*kernelsizex)))*3+0]*ptrGpu_arrayFilter[dx+2]);		//now these change
			sumy += (shared_mem[(shared_index_x+((shared_index_y)*(blocksize+2*kernelsizex)))*3+1]*ptrGpu_arrayFilter[dx+2]);		//TODO
			sumz += (shared_mem[(shared_index_x+((shared_index_y)*(blocksize+2*kernelsizex)))*3+2]*ptrGpu_arrayFilter[dx+2]);
		}
	
	if(x == 0 && y == 0)
		for(int i=0;i<5;i++)
		{
			printf("%d - ", ptrGpu_arrayFilter[i]);
		}
...
void computeImages(int kernelsizex, int kernelsizey)
{
	...
	int arrayFilter[] = {1,4,6,4,1};
	int *ptrGpu_arrayFilter;
	cudaMalloc( (void**)&ptrGpu_arrayFilter, sizeof(arrayFilter));
	cudaMemcpy( ptrGpu_arrayFilter, arrayFilter, sizeof(arrayFilter), cudaMemcpyHostToDevice );
	cudaEventSynchronize(startEvent);
	cudaEventRecord(startEvent, 0);
	filter<<<grid,block>>>(dev_input, dev_bitmap, imagesizex, imagesizey, kernelsizex, kernelsizey, ptrGpu_arrayFilter); // Awful load balance
