...
void runKernel(cl_kernel kernel, int threads, cl_mem data, unsigned int length, unsigned int stride)
{
  ...
  ciErrNum |= clSetKernelArg(kernel, 2, sizeof(cl_uint), (void *) &stride);
...
int find_max_gpu(unsigned int *data, unsigned int length)
{
  ...
  for(unsigned int stride = 1; stride<=length/2; stride*=2)
	  runKernel(gpgpuReduction, length, io_data, length, stride);
