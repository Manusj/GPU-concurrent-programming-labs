...
// CHnage input and output to two arrays
// Indexes are key
__global__ 
...
void Draw()
{
	printf("gImageWidth - %d, gImageHeight - %d\n", gImageWidth, gImageHeight);

	int gridsize = 16;
	int blocksize_x = gImageWidth/gridsize;
	int blocksize_y = gImageHeight/gridsize;
	float timediff;
	...
	const int size = gImageWidth * gImageHeight * 4* sizeof(unsigned char);

	unsigned char *gpu_pixels;
	cudaMalloc((void**)&gpu_pixels, size);
	...
	computeFractal<<<dimGrid, dimBlock>>>(gpu_pixels, gImageWidth, gImageHeight, maxiter);
	cudaDeviceSynchronize();
	cudaEventSynchronize(endEvent);
	cudaEventRecord(endEvent, 0);
	cudaMemcpy( pixels, gpu_pixels, size, cudaMemcpyDeviceToHost); 
